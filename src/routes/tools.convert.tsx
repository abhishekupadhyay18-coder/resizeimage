...
import { cropBitmap, setJpegDpi, type CropRect } from "@/lib/compress-image";
...
type ImageItem = { id: string; file: File; bitmap: ImageBitmap; url: string; rotation: number; crop: CropRect | null };
...
function ImagesToPdfPanel() {
  ...
  const add = async (files: File[]) => { const next: ImageItem[] = []; for (const file of files) { try { const bitmap = await createImageBitmap(file); next.push({ id: `${Date.now()}-${Math.random()}`, file, bitmap, url: URL.createObjectURL(file), rotation: 0, crop: null }); } catch { /* unsupported image */ } } setItems((x) => [...x, ...next]); };
  const rotate = (id: string) => setItems((xs) => xs.map((x) => x.id === id ? { ...x, rotation: (x.rotation + 90) % 360 } : x));
  const crop = (id: string) => setItems((xs) => xs.map((x) => x.id === id ? { ...x, crop: x.crop ?? { x: x.bitmap.width * 0.05, y: x.bitmap.height * 0.05, w: x.bitmap.width * 0.9, h: x.bitmap.height * 0.9 } } : x));
  ...
  const run = async () => { setBusy(true); setError(null); try { const doc = await PDFDocument.create(); for (const item of items) { const source = item.crop ? await cropBitmap(item.bitmap, item.crop) : item.bitmap; const c = document.createElement("canvas"); const rotated = item.rotation % 180 !== 0; c.width = rotated ? source.height : source.width; c.height = rotated ? source.width : source.height; const ctx = c.getContext("2d"); if (!ctx) throw new Error("Canvas is unavailable"); ctx.translate(c.width / 2, c.height / 2); ctx.rotate(item.rotation * Math.PI / 180); ctx.drawImage(source, -source.width / 2, -source.height / 2); const blob = await canvasToBlob(c, "image/jpeg", 0.94); const img = await doc.embedJpg(await blob.arrayBuffer()); const [pw, ph] = pageSize === "a4" ? [595, 842] : pageSize === "letter" ? [612, 792] : [img.width / dpi * 72, img.height / dpi * 72]; const page = doc.addPage([pw, ph]); const scale = pageSize === "image" ? 1 : Math.min((pw - 48) / img.width, (ph - 48) / img.height); page.drawImage(img, { x: (pw - img.width * scale) / 2, y: (ph - img.height * scale) / 2, width: img.width * scale, height: img.height * scale }); } downloadBytes(await doc.save(), "images.pdf"); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } };
